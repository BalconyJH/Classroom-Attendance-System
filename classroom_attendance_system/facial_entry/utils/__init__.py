...
async def database_init():
    ...
    try:
        ...
        logger.error(message_translator("ERRORS.DATABASE.INIT.FAILED"))
...
async def init_shape_predictor_model_file():
    ...
    try:
        ...
        logger.info(message_translator("MODEL.LOAD.SUCCESS"))
        _ = None
    except Exception as e:
        logger.error(message_translator("MODEL.LOAD.FAILED"))
        logger.error(e)
        unzip_static(
...
async def init():
    """初始化"""
    await asyncio.gather(database_init(), init_shape_predictor_model_file(), init_face_recognition_model_file())
